...
if [ "$1" = "comparator_tree" ] || [ "$1" = "all" ]
then
    # insert_comparator_tree test 1
    cat comparator_tree.ml >> temp.ml
    echo " 
    let comp x y = x > y;;

    let res = comparator_tree (Node(5, Node(1, Empty, Empty), Empty)) 6 comp;;
 ...
 let rec string_of_tree = function
    ...
    ocamlc -o insert temp.ml
    ocamlrun insert >> results/comparator_tree_res1.txt
    echo -n "number of error in comparator_tree_test1: " >> res
    diff -y --suppress-common-lines test/comparator_tree_test1.txt results/comparator_tree_res1.txt  | wc -l >> res
    cat res
    rm res
    diff -y --suppress-common-lines test/comparator_tree_test1.txt results/comparator_tree_res1.txt 
    rm insert
    rm temp.ml
    rm temp.cmi
    rm temp.cmo
